// VendorModal.jsx - Modal with global search and multi-select for Vendors
...
/**
 * Normalize registered user objects from backend field names to frontend field names.
 * Backend returns: userId, userName, userPhone, userEmail, companies[].companyId
 * Frontend expects: registeredUserId, name, phone, email, companies[].registeredCompanyId
 */
...
export default function VendorModal({ isOpen, onClose, onSave, onDelete, editData, registeredUserConflict, onClearConflict }) {
    ...
    // Global search for Vendors across all users
    useEffect(() => {
        ...
        const debounceTimer = setTimeout(async () => {
            setIsSearching(true);
            try {
                const res = await authFetch(`${API_BASE}/api/vendors/global-search?q=${encodeURIComponent(globalSearchQuery.trim())}&limit=15`);
                if (res && res.ok) {
                    const data = await parseJsonSafe(res);
                    setGlobalSearchResults(normalizeRegisteredUsers(Array.isArray(data) ? data : []));
    ...
    // Fill form with Vendor data (from batch list editing or editData)
    const fillFormWithVendor = (vendor) => {
        setVendorName(vendor.vendorName || "");
        setMobileNumber(vendor.mobileNumber || "");
        setEmailAddress(vendor.emailAddress || "");
        setWebsiteLink(vendor.websiteLink || "");
        setCompanyName(vendor.companyName || "");
        setGstType(vendor.gstType || "Unregistered");
        setGstNumber(vendor.gstNumber || "");
        setBillingAddress(vendor.billingAddress || "");
        setBillingPinCode(vendor.billingPinCode || "");
        setBillingVillage(vendor.billingVillage || "");
        setBillingTehsil(vendor.billingTehsil || "");
        setBillingDistrict(vendor.billingDistrict || "");
        setBillingState(vendor.billingState || "");
        setBillingCountry(vendor.billingCountry || "India");
        setSameAsBilling(vendor.sameAsBilling ?? true);
        setShippingAddress(vendor.shippingAddress || "");
        setShippingPinCode(vendor.shippingPinCode || "");
        setShippingVillage(vendor.shippingVillage || "");
        setShippingTehsil(vendor.shippingTehsil || "");
        setShippingDistrict(vendor.shippingDistrict || "");
        setShippingState(vendor.shippingState || "");
        setShippingCountry(vendor.shippingCountry || "India");
        setOpeningBalanceType(vendor.openingBalanceType || "Credit");
        setOpeningBalanceAmount(vendor.openingBalanceAmount || "");
        // Restore registered user linkage if present
        setRegisteredUserId(vendor.registeredUserId || null);
        setRegisteredCompanyId(vendor.registeredCompanyId || null);
        setIsFromRegistered(vendor.isFromRegistered || false);
    };
    ...
    const fillFormFromRegisteredUser = (user, company) => {
        ...
        if (regType === "regular" || regType === "composition") {
            setGstType(regType.charAt(0).toUpperCase() + regType.slice(1));
            setGstNumber(gst);
        } else if (gst) {
            setGstType("Regular");
            setGstNumber(gst);
        ...
        // Clear shipping (same as billing)
        setShippingAddress("");
        setShippingPinCode("");
        setShippingVillage("");
        setShippingTehsil("");
    ...
    const handleSelectGlobalVendor = (user, company) => {
    ...
    // Add current form data to selected Vendors list
    const handleAddToList = () => {
        ...
        const existsInList = selectedVendors.some(
            (c, idx) => idx !== editingIndex &&
                c.vendorName.toLowerCase() === formData.vendorName.toLowerCase() &&
        ...
        if (editingIndex !== null) {
            ...
            // Add new to list
            setSelectedVendors([...selectedVendors, formData]);
        }

        resetForm();
    };

    // Edit a Vendor from the selected list
    const handleEditFromList = (index) => {
        const vendor = selectedVendors[index];
        fillFormWithVendor(vendor);
        setEditingIndex(index);
    };

    // Remove a Vendor from the selected list
    const handleRemoveFromList = (index) => {
        const updated = selectedVendors.filter((_, i) => i !== index);
        setSelectedVendors(updated);
    ...
    const handleSave = () => {
        if (isEditMode) {
            ...
            const formData = getFormData();
            let vendorsToSave = [...selectedVendors];
            ...
            if (formData.vendorName.trim()) {
                ...
                const existsInList = selectedVendors.some(
                    c => c.vendorName.toLowerCase() === formData.vendorName.toLowerCase() &&
                        (c.companyName || "").toLowerCase() === (formData.companyName || "").toLowerCase()
                );

                if (!existsInList) {
                    vendorsToSave.push(formData);
                }
            }
                    ...
                    <div className="px-6 py-3 bg-gradient-to-r from-gray-50 to-blue-50 border-b border-gray-200">
                        <div className="relative">
                            <label className="block text-xs font-medium text-gray-500 mb-1.5">
                                🔍 Search existing Vendors to auto-fill form (you can add multiple)
                            </label>
                            <div className="relative">
                                <input
                                    ref={globalSearchRef}
                                    ...
                                    placeholder="Type Vendor name, mobile, or company to search..."
                                        ...
                                        <div key={userIdx} className="border-b border-gray-100 last:border-b-0">
                                            ...
                                            {user.companies && user.companies.length > 0 ? (
                                                user.companies.map((company, compIdx) => (
                                                    <button
                                                        key={compIdx}
                                                        onClick={() => handleSelectGlobalVendor(user, company)}
                                                ...
                                                <button
                                                    onClick={() => handleSelectGlobalVendor(user, null)}
                        ...
                        >
                            {isEditMode ? (
                                ...
                                <>
                                    ...
                                    Save {selectedVendors.length > 0 ? `(${selectedVendors.length + (vendorName.trim() ? 1 : 0)})` : ''}
                                </>
                            )}
                        </button>
                    </div>

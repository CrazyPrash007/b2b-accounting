 ...
 */
function PaymentModal({ isOpen, onClose, onSave, onDelete, editData, prefilledParty }) {
    ...
    const handleSave = async () => {
        ...
        try {
            setError("");
            await Promise.resolve(onSave(paymentData, isEditMode));
        } catch (saveErr) {
            const saveMessage = saveErr?.message || "Unable to save payment out entry. Please try again.";
    ...
    const handleSavePayment = async (paymentData, isEdit) => {
        try {
            ...
            if (fields && Array.isArray(fields) && fields.length > 0) {
                ...
                msg = err?.response?.data?.error?.message || err?.response?.data?.message || err?.message || "Failed to save payment. Please check all required fields and try again.";
            }
            throw new Error(msg);
        }
    };

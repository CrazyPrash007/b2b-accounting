...
const create = Joi.object({
    ...
    incomeAmount: Joi.number().precision(2).min(0).required().messages({
        'any.required': 'Income amount is required',
        'number.base': 'Income amount must be a valid number',
        'number.min': 'Income amount cannot be negative'
    }),
    paymentMethod: Joi.string().allow('').optional(),
    category: Joi.string().trim().required().messages({
        'any.required': 'Category is required for income tracking',

...
/**
 * Recomputes totals based on items, discount and additional charges.
 * Returns { taxableAmount, gstAmount, subTotal, total }
 * - items: array with fields qty, rate, gstPercent, gstType
 * - discount: numeric
...
function computeTotalsFromItems(items = [], discount = 0, additionalCharges = [], withGst = true, autoRoundOff = true) {
    ...
    items.forEach(it => {
        ...
        // Safeguard to round to 2 decimals per-line
...
async function getOne(req, res, next) {
    try {
        const ownerId = req.user.ownerId;
        const doc = await Sale.findOne({ _id: req.params.id, ownerId, isDeleted: false }).lean();
...
async function create(req, res, next) {
    try {
        ...
        // enforce owner and audit
        payload.ownerId = ownerId;
        payload.createdBy = req.user.id;

        // Coerce / normalize types
        if (payload.invoiceDate) payload.invoiceDate = new Date(payload.invoiceDate);

        // Normalize items: ensure numeric fields
        ...
        payload.additionalCharges = Array.isArray(payload.additionalCharges) ? payload.additionalCharges.map(c => ({ name: c.name, amount: Number(c.amount) || 0 })) : [];
        payload.payments = Array.isArray(payload.payments) ? payload.payments.map(p => ({ mode: p.mode, amount: Number(p.amount) || 0, refNo: p.refNo || '', depositTo: p.depositTo || '' })) : [];
        ...
        const totals = computeTotalsFromItems(payload.items, payload.discount, payload.additionalCharges, payload.withGst, payload.autoRoundOff);
...
async function update(req, res, next) {
    try {
        ...
        if (payload.payments) {
            payload.payments = payload.payments.map(p => ({ mode: p.mode, amount: Number(p.amount) || 0, refNo: p.refNo || '', depositTo: p.depositTo || '' }));
        }
        ...
        // If items/discount/charges/withGst changed, recompute totals
        ...
        const itemsForCalc = payload.items || existing.items;
        const discountForCalc = (payload.discount != null) ? payload.discount : existing.discount;
        const chargesForCalc = (payload.additionalCharges != null) ? payload.additionalCharges : existing.additionalCharges;
        const withGstForCalc = (payload.withGst != null) ? payload.withGst : existing.withGst;
        const autoRoundOffForCalc = (payload.autoRoundOff != null) ? payload.autoRoundOff : existing.autoRoundOff;

        const totals = computeTotalsFromItems(itemsForCalc, discountForCalc, chargesForCalc, withGstForCalc, autoRoundOffForCalc);
...
async function remove(req, res, next) {
    try {
        const ownerId = req.user.ownerId;
        const id = req.params.id;
        const doc = await Sale.findOneAndUpdate({ _id: id, ownerId }, { isDeleted: true, updatedBy: req.user.id }, { new: true });
        if (!doc) return res.status(404).json({ success: false, error: { message: 'Not found' } });
        res.json({ success: true, data: doc });
    } catch (err) {
        next(err);

...
async function list(req, res, next) {
    try {
        ...
        // allow search on customerName or name
        if (search) {
            q.$or = [
...
async function create(req, res, next) {
    try {
        ...
        // If frontend sends openingBalanceAmount as empty string, convert to 0
...
async function update(req, res, next) {
    try {
        ...
        if (payload.openingBalanceAmount === "" || payload.openingBalanceAmount === null) {
            delete payload.openingBalanceAmount; // don't overwrite with empty string
        }

        const doc = await Customer.findOneAndUpdate({ _id: id, ownerId }, payload, { new: true });
        if (!doc) return res.status(404).json({ success: false, error: { message: 'Not found' } });
        res.json({ success: true, data: doc });
    } catch (err) {
        next(err);
